        ...
        var defaults = {
            ...
            buttons: {
                edit: {
                    ...
                    html: '<span class="icon-pencil2"></span>',
                    action: 'edit'
                },
                delete: {
                    class: 'btn btn-sm btn-default',
                    html: '<span class="icon-trash1"></span>',
                    action: 'delete'
                },
                save: {
                ...
                upload: {
                    class: 'btn btn-sm btn-default',
                    html: '<span class="icon-line-upload"></span>',
                    action: 'upload'
                }
            },
                ...
                toolbar: function() {
                    if (settings.editButton || settings.deleteButton) {
                        ...
                        if ($table.find('th.tabledit-toolbar-column').length === 0) {
                            $table.find('tr:first').append('<th class="tabledit-toolbar-column">Edit</th>');
                        }
...
// Create upload button.
    ...
    uploadButton = '<button type="button" class="tabledit-upload-button ' + settings.buttons.upload.class + '" style="float: none;">' + settings.buttons.upload.html + '</button>';
                        ...
                        var toolbar = '<div class="tabledit-toolbar ' + settings.toolbarClass + '" style="text-align: left; display: inline-block">\n\
                                           <div class="' + settings.groupClass + '" style="float: none;">' + editButton + deleteButton /* + uploadButton */ +'</div>\n\
                                           ' + saveButton + '\n\
                                           ' + confirmButton + '\n\
                                           ' + restoreButton + '\n\
                                       </div></div>';
                        ...
                        // Add toolbar column cells.
                                              if ($table.find('td.edit_toolbar').length === 0) {
                        $table.find('tr:gt(0)').append('<td class="edit_toolbar" style="white-space: nowrap; width: 1%;">' + toolbar + '</td>');
                       }
       ...
       $table.on('click','button.tabledit-upload-button', function(event) {
         ...
         if(event.handled !==true){
            ...
            var $td=$(this).closest('td');
             rowID=$td.closest('tr').find('.tabledit-identifier').text();
