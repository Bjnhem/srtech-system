    ...
    <script>
        $(document).ready(function() {
            ...
            function determineIndexAxis() {
                ...
                // Đổi trục index dựa trên độ rộng màn hình
                indexAxis = screenWidth < 768 ? "y" : "x"; // Màn hình nhỏ: trục y, màn hình lớn: trục x

                // Thay đổi chiều cao của .chart-container dựa trên độ rộng màn hình
                if (screenWidth < 768) {
            ...
            // Gọi hàm mỗi khi thay đổi kích thước màn hình
            $(window).resize(function() {
                determineIndexAxis();
            });

            // Gọi hàm một lần khi trang load
            $(document).ready(function() {
                determineIndexAxis();
            });
            ...
            function show_table_loss_model() {
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        $.each(response.data, function(model, stats) {
                            if (model == 'All Models') {
                                var categoriesOrder = ["Prod [ea]", "Q'ty NG [ea]",
                                    "Target Rate [%]", "Rate [%]"
                                ];
                            } else {
                        ...
                        tables.rows().every(function(rowIdx) {
                            ...
                            if (current_data == modelData2) {
                                var prevRow = tables.row(rowIdx - rowcount)
                                    .node(); // Lấy dòng trước đó
                                rowcount++; // Tăng số dòng cùng giá trị model
                                $(prevRow).find('td:nth-child(1)').attr('rowspan',
                                    rowcount); // Tăng rowspan cho dòng trước
