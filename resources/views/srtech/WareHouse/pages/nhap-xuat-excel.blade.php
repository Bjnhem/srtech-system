...
@section('content')
    <div class="card form-card">
        <div class="card-header">
            <h3 class="mb-0" id="title-header">NHẬP KHO THEO FILE EXCEL</h3>
...
@section('admin-js')
    ...
    <script>
        document.getElementById('form-upload').addEventListener('submit', async function(e) {
            e.preventDefault(); // Ngăn chặn form gửi dữ liệu ngay lập tức
            ...
            try {
                const response = await fetch('{{ route('warehouse.update.product') }}', {
                    method: 'POST',
                    body: formData
                });
                ...
                if (response.ok) {
                    ...
                    // Lặp qua từng dòng dữ liệu và hiển thị kết quả
                    result.data.forEach((item, index) => {
                        const row = document.createElement('tr');

                        // Xử lý các trường và kiểm tra lỗi
                        const errorClass = item.error ? 'table-danger' : '';
                    ...
                    <td>${item.to || ''}</td>
                    <td>${item.quantity || ''}</td>
                    <td class="${errorClass}">${item.error || 'Hợp lệ'}</td>
                `;
                    ...
                    alert('Có lỗi xảy ra khi xử lý file. Vui lòng kiểm tra lại.');
                }
            } catch (error) {
                console.error('Lỗi xử lý file:', error);
                alert('Lỗi khi tải file lên. Vui lòng thử lại sau.');
            }
        });
    </script> --}}
@endsection
